...
local Custom = {} do
  Custom.ColorRGB = Color3.fromRGB(255, 31, 61)
...
function CircleClick(Button, X, Y)
	task.spawn(function()
		...
		Circle.ImageColor3 = Color3.fromRGB(255, 31, 61)
...
function Speed_Library:CreateWindow(Config)
  ...
  -- /// Responsive auto-scale (mobile vs desktop)
  do
    local cam = workspace.CurrentCamera
    local vp = cam and cam.ViewportSize or Vector2.new(1280, 720)
    local baseW, baseH = SizeUi.X.Offset, SizeUi.Y.Offset
    if baseW <= 0 then baseW = 550 end
    if baseH <= 0 then baseH = 315 end
    local isMobile = vp.X < 800 or UserInputService.TouchEnabled and not UserInputService.MouseEnabled
    if isMobile then
      local w = math.floor(math.min(vp.X * 0.92, baseW))
      local h = math.floor(math.min(vp.Y * 0.85, baseH))
      -- preserve aspect ratio if too small
      if w / h > baseW / baseH then w = math.floor(h * (baseW / baseH)) end
      SizeUi = UDim2.fromOffset(w, h)
      TabWidth = math.max(70, math.floor(TabWidth * (w / baseW)))
    end
  end
  ...
  local DropShadow = Custom:Create("ImageLabel", {
    Image = "",
    ImageColor3 = Color3.fromRGB(10, 10, 14),
    ImageTransparency = 0.5,
    ScaleType = Enum.ScaleType.Slice,
    SliceCenter = Rect.new(49, 49, 450, 450),
    AnchorPoint = Vector2.new(0.5, 0.5),
  ...
  Custom:Create("UIStroke", {
    Color = Color3.fromRGB(255, 31, 61),
    Thickness = 1.6
  }, Main)
  ...
  function Tabs:CreateTab(Config)
    ...
    local ScrolLayers = Custom:Create("ScrollingFrame", {
			ScrollBarImageColor3 = Color3.fromRGB(255, 31, 61),
			ScrollBarThickness = 0,
			Active = true,
    ...
    function Sections:AddSection(Title, OpenSection)
      ...
      function Item:AddLine()
        ...
        Custom:Create("UIGradient", {
          Color = ColorSequence.new{
            ColorSequenceKeypoint.new(0, Color3.fromRGB(255, 31, 61)),
            ColorSequenceKeypoint.new(1, Color3.fromRGB(255, 31, 61))
          },
          Rotation = 0,
        }, Line)
